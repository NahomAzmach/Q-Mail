"""
Header-Only AI Email Analyzer using OpenAI.
This module provides a more privacy-conscious approach to email analysis using OpenAI's API,
sending only domain and header information to the AI for analysis (no email body).
"""
import os
import time
import logging
...
def analyze_email_with_openai(email_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Analyze a single email using OpenAI API directly, using ONLY domain and header information.
    ...
    try:
        ...
        # Get email details - ONLY headers, no body content
...
def batch_analyze_emails_with_openai(emails: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Analyze a batch of emails using OpenAI API with header-only analysis.
...
def analyze_email_security(email: Dict[str, Any]) -> Dict[str, Any]:
    """
    Analyze an email for security concerns using header-only OpenAI API.
...
def batch_analyze_emails(emails: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Analyze a batch of emails for security concerns using header-only OpenAI API.
    This is the main function to be called by the application.
